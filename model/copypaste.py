...
class copypaste(nn.Module):
    def __init__(self, dim_input, nmem, lr=1e-3, alpha=0.5, tau=0.1, dropout_rate=0, time_separation=10):
        super(copypaste, self).__init__()

        self.dim_input = self.dim_output = dim_input
        self.dim_hidden = self.dim_memory = self.dim_input * 2
        self.time_separation = time_separation+1
    ...
    def forward(self, X, sceneid, storage):
        ...
        for self.t in range(self.X.shape[1] - 1):
            ...
            self.encode_memory()
            if len(self.storage) > self.time_separation: #1:
        ...
        return loss, self.storage, log_loss, log_acc, log_h, log_m, log_yhat, log_attn, log_m_sc
    ...
    def retrieve_memory(self):
        values = torch.stack([m['v'].detach() for m in self.storage[:-self.time_separation]], dim=0)

        d = self.h_t.size(-1)
        self.attn = (self.h_t.reshape(1, -1) @ values.T) / (self.tau * d ** 0.5)
        self.attn = self.softmax(self.attn[0, :])

        self.m_t = torch.sum(self.attn.unsqueeze(0) @ values, dim=0)
        self.m_sc = self.storage[torch.argmax(self.attn).item()]['scene']

        for i, m in enumerate(self.storage[:-self.time_separation]):
